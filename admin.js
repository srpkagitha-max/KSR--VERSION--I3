...
async function saveExam(){
  try{
    ...
    await setDoc(doc(db,"exams",id),{title:$("examTitle").value||id,questions:qs,startTime:st,endTime:en,passMark:Number($("passMark").value)||35,sec:Number($("sec").value)||45,marks:Number($("marks").value)||1,updatedAt:serverTimestamp(),version:"L3-PHASE2-STABLE"},{merge:true});
...
async function loadResults(id){
  ...
  resultsCache.forEach(r=>h+=`<tr><td>${r.rank}</td><td>${r.name||""}</td><td>${r.phone||""}</td><td>${r.score||0}/${r.total||0}</td><td>${r.pct||""}</td><td class="${r.passStatus==="PASS"?"pass":"fail"}">${r.passStatus}</td><td>${formatTime(r.timeTakenSec)}</td></tr>`);
