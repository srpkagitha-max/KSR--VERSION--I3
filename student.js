...
let EXAM = null, Q = [], cur = 0, ans = [], rev = [];
let sec = 0, totalSec = 0, timer = null;
let student = "", phone = "", eid = "", code = "";
...
$("startExamBtn").addEventListener("click", async () => {
  try {
    ...
    eid = $("examId").value.trim().toUpperCase();
    code = $("examCode").value.trim().toUpperCase();
    ...
    if(cd.data().used) return alert("Code already used");
    ...
    shuf(Q);
    Q.forEach(q => shuf(q.o));

    ans = Array(Q.length).fill(null);
    rev = Array(Q.length).fill(false);

    const perQ = Number(EXAM.sec) || 45;
    const normalSec = Q.length * perQ;
    const endLeft = EXAM.endTime ? Math.max(1, Math.floor((new Date(EXAM.endTime) - now()) / 1000)) : normalSec;

    sec = Math.min(normalSec, endLeft);
    totalSec = sec;

    if($("examTitle")) $("examTitle").textContent = EXAM.title || eid;
    if($("login")) $("login").classList.add("hide");
    if($("exam")) $("exam").classList.remove("hide");
    ...
    timer = setInterval(() => {
      sec--;
      tick();
      if(sec <= 0) submit(true);
    }, 1000);

  } catch(e) {
    alert("Start exam failed: " + e.message);
  } finally {
...
function show(){
  if(!$("qcard")) return;
  const q = Q[cur];
  let h = `<div class="q">${q.q}</div>`;

  q.o.forEach((o, i) => {
    h += `<label class="opt">
      <input type="radio" name="op" ${ans[cur] === i ? "checked" : ""} onchange="window.selectOption(${i})">
      ${String.fromCharCode(65+i)}) ${o.text}
    </label>`;
  });

  $("qcard").innerHTML = h;
  if($("prog")) $("prog").textContent = `Question ${cur+1} of ${Q.length}`;
  palette();
}
...
function palette(){
  ...
  for(let i = 0; i < Q.length; i++){
    h += `<div class="num ${ans[i] !== null ? "ans " : ""}${rev[i] ? "rev " : ""}${i === cur ? "cur" : ""}" onclick="window.gotoQ(${i})">${i+1}</div>`;
  }
  $("palette").innerHTML = h;
}
...
async function submit(auto){
  ...
  try {
    ...
    Q.forEach((q, i) => {
      const selected = ans[i] !== null ? q.o[ans[i]] : null;
      const corr = q.o.find(x => x.correct);
      ...
      details.push({
        ...
        correctText: corr ? corr.text : "",
        isCorrect: !!(selected && selected.correct)
      });
    });

    const score = correct * (Number(EXAM.marks) || 1);
    const total = Q.length * (Number(EXAM.marks) || 1);
    ...
    if($("exam")) $("exam").classList.add("hide");
    if($("result")){
      $("result").classList.remove("hide");
      $("result").innerHTML = `<h2>Submitted Successfully</h2><p>Score: <b>${score}/${total}</b></p><p>Percentage: <b>${pct}</b></p>`;
    }
...
$("hallTicketBtn").addEventListener("click", async () => {
  const name = $("studentName").value.trim() || "Student";
  const ph = $("studentPhone").value.trim() || "-";
  const photoUrl = $("studentPhoto").value.trim() || "";
  const exId = $("examId").value.trim().toUpperCase() || "-";
  const exCode = $("examCode").value.trim().toUpperCase() || "-";
  ...
  const photoHtml = photoUrl
    ? `<img src="${photoUrl}" style="width:110px;height:130px;object-fit:cover;border-radius:10px;border:2px solid #0b57d0">`
    : `<div style="width:110px;height:130px;border:2px dashed #999;border-radius:10px;display:flex;align-items:center;justify-content:center;font-weight:900;color:#777">PHOTO</div>`;
